 <div class="flex p-4 {{$loop->last ? '':'border-b border-gray-300'}} justify-between ">
     <div class="flex">
         <div class="mr-2 flex-shrink-0">
             <a href="{{route('profile',$tweet->user->username)}}">
                 <img src="{{$tweet->user->getAvatar()}}" width="40" height="40" class="rounded-full mr-3">
             </a>
         </div>
         <div>
             <a href="{{route('profile',$tweet->user->username)}}">
                 <h5 class="mb-4"><span class="font-bold " >{{$tweet->user->name}} </span><span class="text-xs text-gray-500">{{$tweet->created_at->diffForHumans()}}</span></h5>
             </a>
             ...
             <div class="flex items-center">
                 <form action="/tweets/{{$tweet->id}}/like" method="post">
                     @csrf
                     <div type="submit" class="flex items-center mr-4 {{$tweet->isLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
                         <button type="submit">
                             <svg class=" mr-1 w-3"
                                  viewBox="0 0 20 20">
                 ...
                 <form action="/tweets/{{$tweet->id}}/like" method="post">
                     ...
                     <div class="flex items-center {{$tweet->isDisLikedBy(auth()->user()) ? 'text-red-500' : 'text-gray-500' }}">
                         ...
                         <span class="text-xs ">{{$tweet->tdislikes ?:0}}</span>
                     </div>
                 </form>
